  ...
  subroutine optmum0(age, A, AIME, W, Cinit, Hinit, M, Vgood, Vbad, Astate, AIMEstate, Wstate, &
    ...
    do lfpi = 0, 1
       if (lfpi==0_8) then
          ...
          & inHstate, mortality_good, mortality_bad, good_to_bad, bad_to_bad, hlogwage, ulogwage, &
          & hhgr, hugr, uhgr, uugr, gvec, ageshift,  H, currentB, PIA, pb, ss, particip, &
          ...
          if (intval==1_8) then
             ...
             if (Hinit<inHstate(2)) then
                ...
                call workval0(age, A, AIME, W, Cinit, M, Vgood, Vbad, Astate, AIMEstate, Wstate, inCstate, &
                & inHstate, mortality_good, mortality_bad, good_to_bad, bad_to_bad, hlogwage, ulogwage, &
                & hhgr, hugr, uhgr, uugr, gvec, ageshift, inHstate(initHi), currentB, PIA, pb, ss, particip, &
                & temp_Copt, temp_Hopt, temp_Aopt, temp_Wopt_good, temp_Wopt_bad, temp_AIMEopt, temp_Iopt, &
                & temp_pbopt, temp_ssopt, valm)
                call workval0(age, A, AIME, W, Cinit, M, Vgood, Vbad, Astate, AIMEstate, Wstate, inCstate, &
                & inHstate, mortality_good, mortality_bad, good_to_bad, bad_to_bad, hlogwage, ulogwage, &
                & hhgr, hugr, uhgr, uugr, gvec, ageshift, inHstate(initHi-1), currentB, PIA, pb, ss, particip, &
                & temp_Copt, temp_Hopt, temp_Aopt, temp_Wopt_good, temp_Wopt_bad, temp_AIMEopt, temp_Iopt, &
