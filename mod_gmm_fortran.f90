...
contains
  ...
  real(8) function gmm_fortran(params)

    real(8), intent(in) :: params(:)

    !real(8), intent(out) :: gmm_fortran
    ...
    if (p_leispref-p_fixcost-500.0_8<=0.0_8) then
       ...
       gmm_fortran = 10000000000000000.0_8
       return
    end if
    ...
    call decrule(Astate, Wstate, AIMEstate, mortality_good, mortality_bad, good_to_bad, &
      & bad_to_bad, hlogwage, ulogwage, hhgr, hugr, uhgr, uugr, gvec, ageshift, &
    ...
    call simulation_prof(A_dist, M_dist, W_dist, AIME_dist, mortality_good, mortality_bad, &
         good_to_bad, bad_to_bad, hlogwage, ulogwage, hhgr, hugr, uhgr, uugr, gvec, ageshift, &
    ...
    smoments(momnum*4+1:momnum*5,1) = mean_prof_A(30-bornage+1:69-bornage+1)/1000000
    smoments(momnum*5+1:momnum*6,1) = med_prof_A(30-bornage+1:69-bornage+1)/1000000
    ...
    gmm_fortran = gmm_mat(1, 1)

    write(*,*) 'gmm', gmm_fortran

    return
  end function gmm_fortran

end module mod_gmm_fortran
