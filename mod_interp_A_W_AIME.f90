...
contains
  ...
  function interp(age, A, W, AIME, V, Astate, Wstate, AIMEstate, Bi) result(Vinterp)
    ...
    if (Aidx == Anum) then
       if (AIME == AIMEnum) then
          ...
          if (Widx == Wnum) then
             Vinterp = Afrac*(1.0_8-AIMEfrac)*Wfrac*V(ageadj+1, AIMEidx, Widx, Aidx, Bidx) + Afrac*AIMEfrac*Wfrac*V(ageadj+1, AIMEidx+1, Widx, Aidx, Bidx)
          else
             Vinterp = Afrac*(1.0_8-AIMEfrac)*((1.0_8-Wfrac)*V(ageadj+1, AIMEidx, Widx, Aidx, Bidx) + Wfrac*V(ageadj+1, AIMEidx, Widx+1, Aidx, Bidx)) &
                  + Afrac*AIMEfrac*((1.0_8-Wfrac)*V(ageadj+1, AIMEidx+1, Widx, Aidx, Bidx) + Wfrac*V(ageadj+1, AIMEidx+1, Widx+1, Aidx, Bidx))
          end if
       end if
    else
       if (AIME == AIMEnum) then
          if (Widx == Wnum) then
             Vinterp = (1.0_8-Afrac)*AIMEfrac*Wfrac*V(ageadj+1, AIMEidx, Widx, Aidx, Bidx) + Afrac*AIMEfrac*Wfrac*V(ageadj+1, AIMEidx, Widx, Aidx+1, Bidx)
          else
             Vinterp = (1.0_8-Afrac)*AIMEfrac*((1.0_8-Wfrac)*V(ageadj+1, AIMEidx, Widx, Aidx, Bidx) + Wfrac*V(ageadj+1, AIMEidx, Widx+1, Aidx, Bidx)) &
                  + Afrac*AIMEfrac*((1.0_8-Wfrac)*V(ageadj+1, AIMEidx, Widx, Aidx+1, Bidx) + Wfrac*V(ageadj+1, AIMEidx, Widx+1, Aidx+1, Bidx))
          end if
       else
          if (Widx == Wnum) then
             Vinterp = (1.0_8-Afrac)*(1.0_8-AIMEfrac)*Wfrac*V(ageadj+1, AIMEidx, Widx, Aidx, Bidx) + (1.0_8-Afrac)*AIMEfrac*Wfrac*V(ageadj+1, AIMEidx+1, Widx, Aidx, Bidx) &
                  + Afrac*(1.0_8-AIMEfrac)*Wfrac*V(ageadj+1, AIMEidx, Widx, Aidx+1, Bidx) + Afrac*AIMEfrac*Wfrac*V(ageadj+1, AIMEidx+1, Widx, Aidx+1, Bidx) 
          else
             Vinterp = (1.0_8-Afrac)*(1.0_8-AIMEfrac)*((1.0_8-Wfrac)*V(ageadj+1, AIMEidx, Widx, Aidx, Bidx) + Wfrac*V(ageadj+1, AIMEidx, Widx+1, Aidx, Bidx)) &
                  + (1.0_8-Afrac)*AIMEfrac*((1.0_8-Wfrac)*V(ageadj+1, AIMEidx+1, Widx, Aidx, Bidx) + Wfrac*V(ageadj+1, AIMEidx+1, Widx+1, Aidx, Bidx)) + &
